...
const Modal: React.FC<ModalProps> = ({
  ...
  const handlePictureUpload = async (
    ...
    if (type === "profile") {
      let response = await fetch(
        server + "api/users/user/" + user.user_id + "/image",
        {
          method: "PUT",
          body: form,
        }
  ...
  const handleLapSubmit: React.ChangeEventHandler<HTMLFormElement> = async (
    e
  ) => {
    e.preventDefault();
  ...
  const handleLeagueCreate: React.ChangeEventHandler<HTMLFormElement> = async (
    e
  ) => {
    e.preventDefault();
  ...
  const handleLeagueAddUsers: React.ChangeEventHandler<
    HTMLFormElement
  > = async (e) => {
    e.preventDefault();
    ...
    for (const selection of addSelection) {
      let response = (await fetch(
        server + "api/leagues/" + league_id + "/driver/",
        {
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleLeagueRemoveUsers: React.ChangeEventHandler<
    HTMLFormElement
  > = async (e) => {
    e.preventDefault();
  ...
  const getAllUsers = async () => {
    ...
    if (responseUsers.status === 200) {
      ...
      const responseLeagueUsers = await fetch(
        server + "api/leagues/drivers/" + league_id
      );
      if (responseLeagueUsers.status === 200) {
        ...
        const leagueDrivers = users.filter((u) =>
          driverIdSet.has(Number(u.user_id))
        );
        const nonLeagueUsers = users.filter(
          (u) => !driverIdSet.has(Number(u.user_id))
        );
  ...
  return (
    <>
      ...
      <div
        className="modal"
        style={type === "carAdd" ? { top: "10dvh" } : { top: "15dvh" }}
      >
            ...
            <div className="driver-add-container">
              {dbUsers.length > 0 ? (
                dbUsers
                  ...
                  .map((driver) => {
                    const isSelected = addSelection.some(
                      (d) => d.user_id === driver.user_id
                    );
                    ...
                    return (
                      <p
                        ...
                        className={
                          isSelected ? "modal-driver-selected" : "modal-driver"
                        }
                      >
                        {driver.username}
                      </p>
        ...
        {type === "leagueDetailRemove" && (
          <form onSubmit={handleLeagueRemoveUsers}>
            {removeSelection.length > 0 ? (
              <p>
                {removeSelection.map((driver) => driver.username).join(", ")}
              </p>
            ) : (
              <p>No driver selected</p>
            )}
            ...
            <div className="driver-add-container">
              {drivers.length > 0 ? (
                drivers
                  ...
                  .map((driver) => {
                    const isSelected = removeSelection.some(
                      (d) => d.user_id === driver.user_id
                    );
          ...
          <form onSubmit={handleCarAdd}>
            ...
            <div className="tire-picker-header">
              ...
              <div className="tire-picker-actions">
                <button
                  className="tire-action"
                  type="button"
                  onClick={selectAllTires}
                >
                  Select all
                </button>
                <button
            ...
            <div className="tire-picker">
              {(dbTires ?? []).map((t: any) => {
                ...
                return (
                  <label
                    key={id}
                    className={`tire-chip ${checked ? "checked" : ""}`}
                  >
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={() => toggleTire(id)}
                    />
                    <span>{label}</span>
                  </label>
                );
              })}
              {(!dbTires || dbTires.length === 0) && (
                <p className="tire-empty">No tires available</p>
              )}
            </div>
